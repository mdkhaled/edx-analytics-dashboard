...
class CourseIndexTests(AnalyticsDashboardWebAppTestMixin, WebAppTest):
    ...
    def _test_search(self):
        ...
        # Perform search
        input = self.driver.find_element_by_id('search-course-list')
        input.send_keys(Keys.CONTROL, 'a')  # in-case there is a previous search
        ...
        self.assertEqual(input.get_attribute('value'), 'search')
    ...
    def check_cleared(self):
        ...
        input = self.driver.find_element_by_id('search-course-list')
        self.assertNotEqual(input.get_attribute('value'), 'search')
